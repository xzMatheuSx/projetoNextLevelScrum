...
export default function FormsEditAluno({ alunoId, onSave }: FormsEditAlunoProps) {
  ...
  const onSubmit = async (data: FormData) => {
    const updatedData = Object.fromEntries(
      Object.entries(data).filter(([key, value]) => value !== initialData[key as keyof FormData])
    );

    try {
      // await EditAluno(alunoId, updatedData);
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Input
                      {...field}
                      value={field.value}
                      className={cn('bg-[#1F1F1F] border', errors.horarioEstimadoTreino ? 'border-red-400' : 'border-[#2A2A2A]')}
                    />
                  )}
                />
              </div>
